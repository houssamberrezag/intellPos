...
export class PurchaseBillService {
  ...
  constructor(private settingsService: SettingsService, private purchaseService: PurchaseService) {}
  ...
  async generatePdf(transaction: ITransaction): Promise<void> {
    ...
    await this.purchaseService
      .getPurchasesByReference(transaction.referenceNo ?? '')
      .toPromise()
      .then(res => {
  ...
  getDocumentDefinition(): TDocumentDefinitions {
    return {
      content: [
        ...
        {
          columns: [
            { text: 'Fournisseur', style: 'subTitle' },
            { text: '' },
          ...
          table: {
            ...
            body: [
              ...
              ...this.purchases.map(purchase => [
                { text: purchase.product?.name, style: ['textCenter'] },
                { text: purchase.quantity, style: ['textCenter'] },
                { text: purchase.unitCost?.toFixed(2), style: ['textCenter'] },
                { text: purchase.subTotal?.toFixed(2), style: ['textCenter'] },
              ]),
            ],
          },
        },
