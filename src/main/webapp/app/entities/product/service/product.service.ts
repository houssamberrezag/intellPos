...
export class ProductService {
  ...
  loadProductsAlertWithOutSub(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<IProduct[]>(this.resourceUrl + '/alert', { params: options, observe: 'response' })
      .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
  ...
  protected convertDateFromClient(product: IProduct): IProduct {
    return Object.assign({}, product, {
      ...
      deletedAt: product.deletedAt?.isValid() ? product.deletedAt.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((product: IProduct) => {
        ...
        product.deletedAt = product.deletedAt ? dayjs(product.deletedAt) : undefined;
      });
    }
    return res;
  }
