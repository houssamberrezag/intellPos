...
export class ReturnTransactionComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.returnTransactionService
      ...
      .subscribe(
        (res: HttpResponse<IReturnTransaction[]>) => {
  ...
  trackId(index: number, item: IReturnTransaction): number {
    return item.id!;
  }

  delete(returnTransaction: IReturnTransaction): void {
    const modalRef = this.modalService.open(ReturnTransactionDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.returnTransaction = returnTransaction;
  ...
  protected onSuccess(data: IReturnTransaction[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/return-transaction'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
